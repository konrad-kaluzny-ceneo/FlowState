...
## Expand merge scope (2026-06-13 — flow coherence)

| Proposal | Action in `/10x-plan` |
|----------|----------------------|
| Post-merge authenticated wedge coach (T-05) | Follow-up plan scope — subcopy on first check-in + first suggestion after guest→account merge; flag `hasSeenAuthenticatedWedge`; **no new overlay**; after F-07. Promote from parked P-GAP-102 / guest activation — not a new slice. See [`flow-coherence-recommendations.md`](../flow-coherence-recommendations.md). |
