...
- **Session kickoff suggestion (S-15, FR-021/FR-022)**: `e2e/session-kickoff.spec.ts` — session-start idle with tasks (reload after add to refresh eligibility) asserts kickoff rationale + `suggested-task-row`, accept → pre-focus + `kickoff-duration-chips`, chip tap stages `work-duration-min`/`sec` and `timer-countdown` at start, override via Focus shows `suggestion-override-ack`. Helpers: `e2e/helpers/kickoff.ts` (`waitForKickoffSuggestion`, `expectKickoffVisible`, `acceptKickoffSuggestion`, `expectKickoffDurationChips`); task setup via `addTaskWithAttributes` in `e2e/helpers/work-cycle.ts`. Reference test: `shows kickoff card with rationale and highlighted row on session-start idle`. Run: `set CI=true && pnpm test:e2e e2e/session-kickoff.spec.ts`.
