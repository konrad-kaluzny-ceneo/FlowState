...
- K2: Path C (`useDomainTasks`) before Path A (repo extend for checkIn/suggestion); **char tests before Path C enforcement**
- Ship order after F-07: K2 char → K1 extracts → K2 Path C (cheapest / most independent first)
...
| 3a | K2 char | `data-mode-acl-hardening` (char) | — | F-07 merged | `/10x-new data-mode-acl-hardening` | not started |
| 2 | K1 | `cycle-hook-pure-extracts` | — | F-07 merged; K2 char landed | `/10x-new cycle-hook-pure-extracts` | not started |
| 3b | K2 Path C | `data-mode-acl-hardening` (enforcement) | — | K1 merged | continue `data-mode-acl-hardening` branch | not started |
