...
- [ ] 3.3 Component tests pass (delegation-suggestion-card, task-list, plan-dnia-view)
- [ ] 3.4 i18n key parity holds: `messages-parity.test.ts`
- [ ] 3.5 Full suite passes: `pnpm test`

#### Manual

- [ ] 3.6 Delegation card shows plausible candidate + rationale on Plan dnia
- [ ] 3.7 Accept → task moves to Delegated tab, leaves Fokus pool and Active tab
- [ ] 3.8 Revert circle → task returns to Active with fresh sort position
- [ ] 3.9 Skip → next candidate or empty state; resets next day
- [ ] 3.10 Guest mode shows no delegation card
- [ ] 3.11 Task detail panel shows "Delegated" status pill (EN + PL)
