...
This slice adds nullable `personaPresetId` on `Task` (and guest blob), expands the catalog to **8 user-approved personas** (see locked table below), rewires create UX and `TaskBadges`, and threads the field through tRPC + data-mode repos. Inline edit stays attribute-panel-only (S-29 scope preserved).
...
2. **Catalog** — 8 approved personas; pure helpers: `getPresetById`, `taskAttributesMatchPreset` (effort excluded), label lookup; Lucide icons.
3. **Create UX** — Always-visible effort when a preset is selected; effort change does **not** call `markCreateFormCustom()`; `createTask` sends `personaPresetId`; picker layout for 8 chips (+ Custom).
...
- **Create:** Set when user adds via preset chip (including effort-only override). Set `"custom"` when user chose Custom. Set `null` when no preset selected (legacy path only).
...
| --- | --- |
| `personaPresetId == null` | Legacy: work type + `U:`/`I:` (+ ASAP) — same as today (pre-migration / no persona) |
...
| User chose Custom at create (`personaPresetId === "custom"`) | **Custom** label + simplified F-05 (work type + `U:`/`I:`) |
...
| Unknown / removed catalog id | **Custom** + F-05 detail (defensive — do not masquerade as legacy) |
...
| Risk | Mitigation |
| --- | --- |
| 10 chips crowd mobile create form | Horizontal scroll or two-row wrap in `PersonaPresetPicker`; short labels (Focus, Synchro, …). |
...
| Invalid stored id | `getPresetById` returns undefined → **custom-detail** (Custom + F-05), not legacy |
